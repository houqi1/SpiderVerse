Shader "Hidden/SpiderVerse/ObjectLineArtGpu"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct GpuSegment { float4 a,b,previous,next,style; };
            StructuredBuffer<GpuSegment> _GpuSegments;
            Varyings Vert(uint vertexID:SV_VertexID)
            {
                GpuSegment segment = _GpuSegments[vertexID / 6];
                vertexID %= 6;
                uint corner = vertexID == 0 ? 0 : vertexID == 1 ? 1 : vertexID == 2 ? 2 : vertexID == 3 ? 1 : vertexID == 4 ? 3 : 2;
                bool end = corner >= 2; Attributes input;
                input.position = end ? segment.b.xyz : segment.a.xyz;
